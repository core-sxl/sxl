...
bool CKey::VerifyPubKey(const CPubKey &pubkey) const {
    ...
    uint8_t rnd[8];
    std::string str = "SXL key verification\n";
...
string CKeyID::ToAddress() const {
    if (IsNull()) {
        ...
        string address = CCoinAddress(*this).ToString();
        static string prefix = SysCfg().PubkeyAddressPrefix();
...
CKeyID::CKeyID(const string &strAddress) : uint160() {
    static string prefix = SysCfg().PubkeyAddressPrefix();
    static size_t len = prefix.size();
    if (strAddress.compare(0, len, prefix) == 0) {
        string address = strAddress.substr(len);
        CCoinAddress addr(address);
        addr.GetKeyId(*this);
    } else if (strAddress.length() == 40) {
