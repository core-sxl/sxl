...
bool CKeyCombi::GetPubKey(CPubKey& mOutKey, bool hasMinerKey) const {
    if (hasMinerKey) {
        if (mMinerCkey.IsValid()) {
...
bool CKeyStore::GetPubKey(const CKeyID& address, CPubKey& vchPubKeyOut, bool hasMinerKey) const {
    CKey key;
    if (!GetKey(address, key, hasMinerKey))
        return false;
