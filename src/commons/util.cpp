...
// Copyright (c) 2009-2014 The SXL developers
...
const boost::filesystem::path& GetDataDir(bool fNetSpecific) {
    ...
    std::string strCfgDataDir = CBaseParams::IsArgCount("-datadir") ? CBaseParams::GetArg("-datadir", "") : "";
    if ("" == strCfgDataDir) {
...
void ReadConfigFile(map<string, string>& mapSettingsRet,
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end;
         it != end; ++it) {
        // Don't overwrite existing settings so command line settings override SXL.conf
...
int64_t GetAdjustedTime() { return GetTime(); }
...
void AddTimeData(const CNetAddr& ip, int64_t nTime) {
    ...
    LogPrint("INFO", "Added time data, samples %d, offset %+d seconds\n", vTimeOffsets.size(), nOffsetSample);

    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1) {
        int64_t nMedian         = vTimeOffsets.median();
        vector<int64_t> vSorted = vTimeOffsets.sorted();

        // As block interval is so short, i.e., 10 seconds. It's not necessary to adjust out timestamp depending on
        // other peers. Every block producer should make sure it's timestamp is exactly precise. Of course, if nobody
            ...
            if (!fDone) {
                ...
                if (!fMatch) {
                    ...
                    string strMessage =
                        ...
                        "are correct! If your clock is wrong, coind will not work properly.");
...
boost::filesystem::path GetTempPath() {
    ...
    boost::filesystem::path path = boost::filesystem::path("/tmp");
