...
string EncodeBase58(const vector<unsigned char>& vch) { return EncodeBase58(&vch[0], &vch[0] + vch.size()); }

bool DecodeBase58(const string& str, vector<unsigned char>& vchRet) { return DecodeBase58(str.c_str(), vchRet); }
...
bool CBase58Data::SetString(const string& str, uint32_t nVersionBytes) { return SetString(str.c_str(), nVersionBytes); }
...
bool CCoinSecret::SetString(const char* pszSecret) { return CBase58Data::SetString(pszSecret, 1) && IsValid(); }

bool CCoinSecret::SetString(const string& strSecret) { return SetString(strSecret.c_str()); }
