...
public:
    ...
    string ToString() const {
        return strprintf("CBlockIndex(pprev=%p, height=%d, merkle=%s, blockHash=%s)", pprev, height,
                         merkleRootHash.ToString(), GetBlockHash().ToString());
    }

    string GetIndentityString() const { return strprintf("%d:%s", height, GetBlockHash().ToString()); }
...
public:
    ...
    IMPLEMENT_SERIALIZE(
        ...
        READWRITE(VARINT(nFuel));
        READWRITE(VARINT(nFuelRate));
        READWRITE(vSignature);)
