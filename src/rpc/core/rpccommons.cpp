...
bool ParseRpcInputMoney(const string &comboMoneyStr, ComboMoney &comboMoney, const TokenSymbol defaultSymbol) {
    ...
    switch (comboMoneyArr.size()) {
        case 1: {
            ...
            comboMoney.unit   = COIN_UNIT::SASX;
            break;
        }
        case 2: {
            if (is_number(comboMoneyArr[0])) {
                ...
                if (comboMoneyArr[0].size() > MAX_ASSET_SYMBOL_LEN
) // check symbol len
                    return false;
...
ComboMoney RPC_PARAM::GetFee(const Array& params, const size_t index, const TxType txType) {
    ...
    if (params.size() > index) {
        fee = GetComboMoney(params[index], SYMB::SXL);
        ...
        if (fee.GetSawiAmount() < minFee)
            throw JSONRPCError(RPC_INVALID_PARAMS,
                strprintf("The given fee is too small: %llu < %llu savl", fee.amount, minFee));
    } else {
        uint64_t minFee;
        if (!GetTxMinFee(txType, chainActive.Height(), SYMB::SXL, minFee))
            throw JSONRPCError(RPC_INVALID_PARAMS,
                strprintf("Can not find the min tx fee! symbol=%s", SYMB::SXL));
        fee.symbol = SYMB::SXL;
        fee.amount = minFee;
        fee.unit = COIN_UNIT::SASX;
    }

    return fee;
}

uint64_t RPC_PARAM::GetDefaultFee(const Array& params, const size_t index, const TxType txType) {
    uint64_t fee, minFee;
    if (!GetTxMinFee(txType, chainActive.Height(), SYMB::SXL, minFee))
        throw JSONRPCError(RPC_INVALID_PARAMS,
            strprintf("Can not find the min tx fee! symbol=%s", SYMB::SXL));
    if (params.size() > index) {
        ...
        if (fee < minFee)
            throw JSONRPCError(RPC_INVALID_PARAMS,
                strprintf("The given fee is too small: %llu < %llu savl", fee, minFee));
    } else {
        fee = minFee;
    }
...
CRegID RPC_PARAM::GetRegId(const Value &jsonValue) {
    auto pUserId = CUserID::ParseUserId(jsonValue.get_str());
    if (!pUserId) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    }
...
TokenSymbol RPC_PARAM::GetAssetIssueSymbol(const Value &jsonValue) {
    ...
    if (symbol.empty() || symbol.size() > MAX_ASSET_SYMBOL_LEN
)
        throw JSONRPCError(RPC_INVALID_PARAMS,
