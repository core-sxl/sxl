...
CAppCFund::CAppCFund(const vector<uint8_t>& tagIn, const uint64_t valueIn, const int32_t timeoutHeightIn) {
    tag           = tagIn;
    value         = valueIn;
...
Object CAppCFund::ToJson() const {
    ...
    result.push_back(Pair("tag",            HexStr(tag)));
...
Object CAppFundOperate::ToJson() const {
    ...
    result.push_back(Pair("uid",            HexStr(GetAppUserV())));
    result.push_back(Pair("tag",            HexStr(GetFundTagV())));
    result.push_back(Pair("opType",         opTypes[opType]));
    result.push_back(Pair("timeoutHeight",  (int32_t)timeoutHeight));
    result.push_back(Pair("money",          money));
