...
std::shared_ptr<string>  CLuaVMRunEnv::ExecuteContract(CLuaVMContext *pContextIn, uint64_t& uRunStep) {
    ...
    // CheckAppAcctOperate only support to check when the transfer symbol is SXL
    if (isCheckAccount && p_context->transfer_symbol == SYMB::SXL && !CheckAppAcctOperate()) {
...
bool CLuaVMRunEnv::CheckAppAcctOperate() {
    assert(p_context->transfer_symbol == SYMB::SXL);
    ...
    for (auto vOpItem : mapAppFundOperate) {
        for (auto appFund : vOpItem.second) {
            if (ADD_FREE_OP == appFund.opType || ADD_TAG_OP == appFund.opType) {
                /*
                int64_t temp = appFund.money;
                temp += addValue;
                if(temp < addValue || temp<appFund.money) {
                    return false;
                }
                */
                int64_t temp = 0;
                if (!SafeAdd(appFund.money, addValue, temp))
                    return false;
                ...
                int64_t temp = appFund.money;
                temp += minusValue;
                if(temp < minusValue || temp<appFund.money) {
                    return false;
                }
                */
                int64_t temp = 0;
                if (!SafeAdd(appFund.money, minusValue, temp))
                    return false;
    ...
    */
    ...
    if (sumValue != sysAcctSum) {
        LogPrint("vm",
                 ...
                 addValue, minusValue, SYMB::SXL, p_context->transfer_amount, sysContractAcct, sumValue, sysAcctSum);
...
bool CLuaVMRunEnv::OperateAccount(const vector<CVmOperate>& operates) {
    ...
    for (auto& operate : operates) {
        ...
        if (!pAccount->OperateBalance(SYMB::SXL, operate.opType, value)) {
        ...
        if (operate.opType == BalanceOpType::ADD_FREE) {
            receipts.emplace_back(nullId, uid, SYMB::SXL, value, ReceiptCode::CONTRACT_ACCOUNT_OPERATE_ADD);
        } else if (operate.opType == BalanceOpType::SUB_FREE) {
            receipts.emplace_back(uid, nullId, SYMB::SXL, value, ReceiptCode::CONTRACT_ACCOUNT_OPERATE_SUB);
        }
...
CContractDBCache* CLuaVMRunEnv::GetContractDB() { return &p_context->p_cw->contractCache; }
