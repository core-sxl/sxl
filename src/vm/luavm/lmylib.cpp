...
int32_t ExGetTxRegIDFunc(lua_State *L) {
    ...
    if (GetTransaction(pBaseTx, hash, pVmRunEnv->GetCw()->blockCache, false)) {
        if (pBaseTx->nTxType == COIN_TRANSFER_TX) {
            CCoinTransferTx *tx = static_cast<CCoinTransferTx*>(pBaseTx.get());
...
int32_t ExGetAccountPublickeyFunc(lua_State *L) {
    ...
    CAccount account;
    if (!pVmRunEnv->GetCatchView()->GetAccount(uid, account)) {
...
int32_t ExQueryAccountBalanceFunc(lua_State *L) {
    ...
    if (!pVmRunEnv->GetCatchView()->GetAccount(uid, account)) {
        ...
        uint64_t nbalance = account.GetToken(SYMB::SXL).free_amount;
        CDataStream tep(SER_DISK, CLIENT_VERSION);
        tep << nbalance;
        vector<uint8_t> TMP(tep.begin(), tep.end());
...
int32_t ExWriteDataDBFunc(lua_State *L) {
    ...
    bool flag = true;
    CContractDBCache* contractDB = pVmRunEnv->GetContractDB();
    string oldValue;
    // TODO: get old data when set data ??
    contractDB->GetContractData(contractRegId, key, oldValue);
    if (!contractDB->SetContractData(contractRegId, key, value)) {
...
int32_t ExDeleteDataDBFunc(lua_State *L) {
    ...
    if (!contractDB->EraseContractData(contractRegId, key)) {
...
int32_t ExReadDataDBFunc(lua_State *L) {
    ...
    CContractDBCache* contractDB = pVmRunEnv->GetContractDB();
    int32_t len = 0;
    if (!contractDB->GetContractData(scriptRegId, key, value)) {
        len = 0;
...
int32_t ExModifyDataDBFunc(lua_State *L) {
    ...
    string oldValue;
    bool flag = false;
    if (contractDB->GetContractData(contractRegId, key, oldValue)) {
        if (contractDB->SetContractData(contractRegId, key, newValue)) {
...
static bool GetDataTableWriteOutput(lua_State *L, CVmOperate &operate) {
    ...
    if (!(getNumberInTable(L, "timeoutHeight", doubleValue))) {
        LogPrint("vm", "WriteOutput(),  get timeoutHeight failed\n");
        return false;
    } else {
 ...
 *      moneySymbol: (string, optional)   money symbol, must be valid symbol, such as SXL, default is SXL
 * }
...
int32_t ExGetContractDataFunc(lua_State *L) {
    ...
    int32_t len = 0;
    if (!contractDB->GetContractData(contractRegId, key, value)) {
        len = 0;
        lua_BurnStoreUnchanged(L, key.size(), 0, BURN_VER_R2);
    } else {
        lua_BurnStoreGet(L, key.size(), value.size(), BURN_VER_R2);
...
int32_t ExWriteOutAppOperateFunc(lua_State *L) {
    ...
    while (count--) {
        ...
        if (temp.money < 0)  // in case contract uses negative money input
...
int32_t ExTransferContractAsset(lua_State *L) {
    ...
    CContractDBCache* pContractScript = pVmRunEnv->GetContractDB();
    ...
    uint64_t money = temp.get()->GetBcoins();
...
int32_t ExGetBlockTimestamp(lua_State *L) {
    ...
    lua_Integer blockTime = vmContext.block_time;
 ...
 *   tokenType: (string, required)         Token type of the transfer, such as SXL
 * }
