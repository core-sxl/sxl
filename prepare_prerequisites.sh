...
case "$ARCH" in
  "Linux")
    if [ ! -e /etc/os-release ]; then
      printf "\\nSXL Core currently supports Ubuntu, Centos Linux only.\\n"
    ...
    case "$OS_NAME" in
      "Ubuntu")
        if [ "${OS_MAJ}" -lt 14 ]; then
          printf "You must be running Ubuntu 14.x or higher to install SXL Core.\\n"
          printf "Exiting now.\\n"
          exit 1
        fi

        if [ $OS_MAJ -gt 18 ]; then
          printf "You must be running Ubuntu 18.x or lower to install SXL Core.\\n"
          printf "Exiting now.\\n"
          exit 1
        fi
      ...
      "CentOS Linux")
        if [ "${OS_MAJ}" -lt 7 ]; then
          printf "You must be running CentOS 7.x to install SXL Core.\\n"
          printf "Exiting now.\\n"
          exit 1
        fi
